...
See recent [publication in Nature Communications](https://www.nature.com/articles/s41467-022-35621-7)! 
...
* vg: [https://github.com/vgteam/vg](https://github.com/vgteam/vg "vg").  Version >v1.47.0. is requried.
...
All downstream processing is based on genome-scale multi-sequence alignments that are converted into graph objects in [GFA format](https://github.com/GFA-spec/GFA-spec/blob/master/GFA1.md).  The sequences used to build the alignments must be represented as paths in the GFA.  Our original method of alignment was based on a multi-threaded implementation of the progressiveMauve algorithm, see [GPA](https://academic.oup.com/bioinformatics/article/35/14/i71/5529231).  Treatment of resultant alignment files - XMFAs - and conversion to GFAs is detailed directly below.  Other software, such as Minigraph-Cactus (see below) and ([pggb](https://github.com/pangenome/pggb), can generate such graphs directly.  In our hands, we have noticed that pggb often generates unwarranted cycles that violate the positional homology paradigm followed in PanPipes.  

We currently recommend [Minigraph-Cactus](https://github.com/ComparativeGenomicsToolkit/cactus/blob/master/doc/pangenome.md) to directly generate GFAs.  Though the resultant GFAs can still contain cycles, these are generally rare and limited to local tandem duplication.  In addition, Mingraph-Cactus is highly scalable. Still, the availability of an excellent [viewer](https://darlinglab.org/mauve/user-guide/viewer.html) for XMFAs makes the choice of alignment strategy difficult.  For alignments of less than 5 genomes within the same species, we continue to recommend the mauve/XMFA approach, but are actively working on GFA to XMFA conversions that will shift the balance to exclusive use of Minigraph-Cactus.  Regardless, if using Minigraph-Cactus, [node chopping](https://github.com/ComparativeGenomicsToolkit/cactus/blob/master/doc/pangenome.md#node-chopping) is a critical parameter for making resultant short read alignments compatible with this pipeline.
...
This tool will convert an maf file of a single sequence/chromosome to an xmfa file, which the mauve-viewer or Geneious can display.  MAF files are produced by default in chromosome subfolder when running minigraph-cactus.  The MAF files can be converted from HAL files using cactus-hal2maf distributed with minigraph-cactus.
